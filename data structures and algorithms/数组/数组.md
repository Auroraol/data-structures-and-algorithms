...
注意数组的长度必须大于等于3，否则可能会导致索引越界错误。
    
for (int i =  数组长度 -1; i >= 2; i++) {
    cout << arr[i];
    cout << arr[i-1];
    cout << arr[i-2];
}
...
==**数组至少有两个元素, 一个元素不进入循环, 需要特判一个元素的情况**==
...
```c++
//的目的是在 l1 和 l2 中至少还有一个节点不为空的情况下执行循环
while (l1 && l2) 
{
...
N个数字 每组X个 （X >= 2） 所以 N % X = 0 每组数字相同 假设有5个数字1 每组2个 5个1 没法分组 所以 每个数字的个数（Times） % X = 0 所以 X 是 所有Times 的约数 所以X是所有Times 的最大公约数 gcd(x, y) C++自带的 求x,y的最大公约数

```c++
class Solution {
public:
    bool hasGroupsSizeX(vector<int>& deck) {
        int counts[10000] = {0};  //辅助数组
        int rs = 0;
        for (int d : deck) 
        {
            counts[d]++;
...
/*
利用lambda表达式
利用pair的默认比较大小的方式: 先按first比较，如果相等，再按照second比较。
*/
class Solution {
public:
...
public:
	...
	bool paix(string& a, string& b)
    {
		...
		*/
		...
		return p.find(a[i]) < p.find(b[i]);	  //按order排序规则
	}
...
{
    if (vec[i] >= mx) 
    	tmp.push_back(vec[i]); //将大于等于最大值的元素添加到临时向量 tmp 
    mx = max(mx, vec[i]);  //最大值
}
...
```c++
  int totalSteps(vector<int>& nums) {
    ...
    vector<int> cnt(n, 0);  //记录截止到当前位置的最长递增子序列的长度
    int count = 0;
    ...
    { 
        ...
        for (int j = i - 1; j >= 0; j--) 
        {
            ...
            mx = max(mx, cnt[j]); //使用mx来记录前面元素中的最大长度
        }
...
class Solution {
    ...
    int minDeletionSize(vector<string>& strs) {
        ...
        删除 不是按字典序升序排列的 列
        */
        const int rows = strs.size();  // row > 1
        const int cols = strs[0].size();
        int res = 0;  // row = 1时,预期结果正好是0
        for (int col = 0; col < cols; ++col) 
        {//列
...
![image-20230818150214989](数组.assets/image-20230818150214989.png)
...
public:
    vector<int> addTwoNum(vector<int>& num1, vector<int>& num2) {
        ...
        {
            int x = i1 >= 0 ? nums1[i1] : 0;
            int y = i2 >= 0 ? nums2[i2] : 0;

            int sum = x + y + carry;
            carry = sum / 10;  //进位数
            
            res.push_back(sum % 10);  //保存结果   
...
public:
    vector<int> luckyNumbers (vector<vector<int>>& matrix) {        
        ...
        vector<int> r(rows, INT_MAX); // row min
        vector<int> c(cols, INT_MIN); // col max
